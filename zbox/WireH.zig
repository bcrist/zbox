...
pub fn turn(self: *WireH) *WireV {
    ...
    if (values.isConstrained(self._y)) {
        self.drawing.constrain(&item._y.begin, .{ .copy = &self._y }, "wire turn");
    } else {
        self.drawing.constrain(&self._y, .{ .copy = &item._y.begin }, "wire turn");
    }
    // Constrain the x coordinates so that the wire will be continuous:
    // TODO bidirectional copy constraints
    if (self._x.isFullyConstrained()) {
        self.drawing.constrain(&item._x, .{ .copy = &self._x.end }, "wire turn");
    } else {
        self.drawing.constrain(&self._x.end, .{ .copy = &item._x }, "wire turn");
    }
    return item;
}
...
pub fn endAt(self: *WireH, x: XRef) *WireH {
    // TODO bidirectional copy constraints
    self.drawing.constrain(&self._x.end, .{ .copy = x._x }, "wire endAt");
    return self;
}
...
pub fn endAtPoint(self: *WireH, end: PointRef) *WireH {
    ...
    // TODO bidirectional copy constraints
    if (values.isConstrained(self._y)) {
        self.drawing.constrain(&end._y, .{ .copy = &self._y }, "wire endAtPoint");
    } else {
        self.drawing.constrain(&self._y, .{ .copy = &end._y }, "wire endAtPoint");
    }
    return self;
}
...
pub fn addMissingConstraints(self: *WireH) void {
    ...
    self._x.addMissingConstraints(self.drawing, 0, self.drawing.style.default_wire_length);
}
